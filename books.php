...
class BookManager {
    ...
    public function getAllBooks($filters = []) {
        ...
        if (!empty($filters['search'])) {
            $sql .= " AND (title LIKE ? OR author LIKE ? OR book_number LIKE ?)";
    ...
    public function addBook($data, $files) {
        ...
        $required = ['title', 'author', 'book_number', 'department'];
        ...
        $stmt = $this->pdo->prepare("SELECT id FROM books WHERE book_number = ?");
        $stmt->execute([$data['book_number']]);
        if ($stmt->fetch()) {
            jsonResponse(false, "Book with this number already exists");
        }
        
        // Handle file uploads
        $coverPath = $this->uploadFile($files['cover_page'], UPLOAD_COVER_PATH, ['jpg', 'jpeg', 'png', 'gif']);
        $pdfPath = $this->uploadFile($files['pdf_content'], UPLOAD_PDF_PATH, ['pdf']);
        
        // Insert book
        $stmt = $this->pdo->prepare("INSERT INTO books (title, author, book_number, edition, department, cover_page, pdf_path) VALUES (?, ?, ?, ?, ?, ?, ?)");
        
        if ($stmt->execute([
            $data['title'], $data['author'], $data['book_number'], 
            $data['edition'] ?? '', $data['department'], $coverPath, $pdfPath
        ])) {
    ...
    public function updateBook($id, $data, $files = []) {
        requireRole(['admin', 'librarian']);
        ...
        $coverPath = $book['cover_page'];
        $pdfPath = $book['pdf_path'];
        
        if (!empty($files['cover_page']['name'])) {
            $coverPath = $this->uploadFile($files['cover_page'], UPLOAD_COVER_PATH, ['jpg', 'jpeg', 'png', 'gif']);
        }
        
        if (!empty($files['pdf_content']['name'])) {
            $pdfPath = $this->uploadFile($files['pdf_content'], UPLOAD_PDF_PATH, ['pdf']);
        }
        
        $stmt = $this->pdo->prepare("UPDATE books SET title = ?, author = ?, book_number = ?, edition = ?, department = ?, cover_page = ?, pdf_path = ? WHERE id = ?");
        
        if ($stmt->execute([
            $data['title'], $data['author'], $data['book_number'], 
            $data['edition'] ?? '', $data['department'], $coverPath, $pdfPath, $id
        ])) {
            jsonResponse(true, 'Book updated successfully');
        } else {
            jsonResponse(false, 'Failed to update book');
        }
    }
    ...
    public function deleteBook($id) {
        ...
        if ($book['cover_page'] && file_exists($book['cover_page'])) {
            unlink($book['cover_page']);
        }
    ...
    private function uploadFile($file, $targetDir, $allowedTypes) {
        ...
        if (!in_array($fileExtension, $allowedTypes)) {
            jsonResponse(false, 'Invalid file type. Allowed: ' . implode(', ', $allowedTypes));
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        case 'get_books':
            ...
            $books = $bookManager->getAllBooks($filters);
        ...
        case 'get_book':
            ...
            $book = $bookManager->getBookById($_POST['id']);
            jsonResponse(true, '', $book);
            break;
            
        case 'add_book':
            $bookManager->addBook($_POST, $_FILES);
            break;
        ...
        case 'update_book':
            ...
            $bookManager->updateBook($_POST['id'], $_POST, $_FILES);
            break;
