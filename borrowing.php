...
class BorrowingManager {
    ...
    public function requestBorrow($bookId, $studentId) {
        requireLogin();
        
        // Check if book exists and is available
        $book = $this->pdo->prepare("SELECT * FROM books WHERE id = ? AND status = 'available'");
        $book->execute([$bookId]);
        $book = $book->fetch(PDO::FETCH_ASSOC);
        
        if (!$book) {
            jsonResponse(false, 'Book not available for borrowing');
        }
        
        // Check if student already has a pending request for this book
        $existingRequest = $this->pdo->prepare("SELECT id FROM borrowings WHERE student_id = ? AND book_id = ? AND status = 'pending'");
        $existingRequest->execute([$studentId, $bookId]);
        
        if ($existingRequest->fetch()) {
            jsonResponse(false, 'You already have a pending request for this book');
        }
        
        // Calculate dates
        $borrowDate = date('Y-m-d');
        $dueDate = date('Y-m-d', strtotime("+" . BORROW_PERIOD_DAYS . " days"));
        
        $stmt = $this->pdo->prepare("INSERT INTO borrowings (student_id, book_id, borrow_date, due_date, status) VALUES (?, ?, ?, ?, 'pending')");
        
        if ($stmt->execute([$studentId, $bookId, $borrowDate, $dueDate])) {
            jsonResponse(true, 'Borrow request submitted successfully');
        } else {
            jsonResponse(false, 'Failed to submit borrow request');
        }
    }
    
    public function getBorrowRequests($filters = []) {
        requireRole(['admin', 'librarian']);
        
        $sql = "SELECT b.*, u.full_name as student_name, u.email as student_email, 
    ...
    public function getUserBorrowings($userId) {
        ...
        $sql = "SELECT b.*, bk.title, bk.author, bk.cover_page
                FROM borrowings b
                JOIN books bk ON b.book_id = bk.id
                WHERE b.student_id = ?
    ...
    public function approveBorrowRequest($requestId) {
        ...
        try {
            // Get borrow request
            $stmt = $this->pdo->prepare("SELECT * FROM borrowings WHERE id = ? AND status = 'pending'");
            ...
            $stmt = $this->pdo->prepare("UPDATE borrowings SET status = 'approved' WHERE id = ?");
            $stmt->execute([$requestId]);
            ...
            $this->pdo->commit();
            jsonResponse(true, 'Borrow request approved successfully');
    ...
    public function rejectBorrowRequest($requestId) {
        ...
        $stmt = $this->pdo->prepare("UPDATE borrowings SET status = 'rejected' WHERE id = ?");
        
        if ($stmt->execute([$requestId])) {
            jsonResponse(true, 'Borrow request rejected');
        } else {
            jsonResponse(false, 'Failed to reject borrow request');
        }
    }
    
    public function returnBook($borrowingId, $userId) {
        requireLogin();
        
        // Verify ownership
        $stmt = $this->pdo->prepare("SELECT * FROM borrowings WHERE id = ? AND student_id = ? AND status = 'approved'");
        $stmt->execute([$borrowingId, $userId]);
        $borrowing = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$borrowing) {
            jsonResponse(false, 'Borrowing record not found');
        }
        ...
        try {
            ...
            $stmt = $this->pdo->prepare("UPDATE borrowings SET status = 'returned', return_date = ? WHERE id = ?");
            $stmt->execute([date('Y-m-d'), $borrowingId]);
            ...
            $this->pdo->commit();
            jsonResponse(true, 'Book returned successfully');
            
        } catch (Exception $e) {
            $this->pdo->rollBack();
            jsonResponse(false, 'Failed to return book');
        }
    }
    
    public function cancelBorrowRequest($requestId, $userId) {
        requireLogin();
        
        $stmt = $this->pdo->prepare("DELETE FROM borrowings WHERE id = ? AND student_id = ? AND status = 'pending'");
        
        if ($stmt->execute([$requestId, $userId])) {
            if ($stmt->rowCount() > 0) {
                jsonResponse(true, 'Borrow request cancelled');
            } else {
                jsonResponse(false, 'Cannot cancel this request');
            }
        } else {
            jsonResponse(false, 'Failed to cancel borrow request');
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $borrowingManager = new BorrowingManager($pdo);
    $action = $_POST['action'] ?? '';
    ...
    switch ($action) {
        case 'request_borrow':
            ...
            $borrowingManager->requestBorrow($_POST['book_id'], $_SESSION['user_id']);
            break;
            
        case 'get_borrow_requests':
            $filters = ['status' => $_POST['status'] ?? ''];
            $requests = $borrowingManager->getBorrowRequests($filters);
            jsonResponse(true, '', $requests);
            break;
            
        case 'get_user_borrowings':
            $borrowings = $borrowingManager->getUserBorrowings($_SESSION['user_id']);
            jsonResponse(true, '', $borrowings);
            break;
        ...
        case 'approve_request':
            ...
            $borrowingManager->approveBorrowRequest($_POST['request_id']);
            break;
        ...
        case 'reject_request':
            ...
            $borrowingManager->rejectBorrowRequest($_POST['request_id']);
            break;
            
        case 'return_book':
            if (empty($_POST['borrowing_id'])) {
                jsonResponse(false, 'Borrowing ID is required');
            }
            $borrowingManager->returnBook($_POST['borrowing_id'], $_SESSION['user_id']);
            break;
        ...
        case 'cancel_request':
            ...
            $borrowingManager->cancelBorrowRequest($_POST['request_id'], $_SESSION['user_id']);
            break;
