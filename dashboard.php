...
class Dashboard {
    ...
    public function getStatistics() {
        ...
        // Total downloads
        $stmt = $this->pdo->prepare("SELECT COUNT(*) as total FROM download_history");
        $stmt->execute();
        $stats['total_downloads'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        
        // Active borrowings
        $stmt = $this->pdo->prepare("SELECT COUNT(*) as total FROM borrowings WHERE status = 'approved'");
        $stmt->execute();
        $stats['active_borrowings'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        
        // Pending requests
        $stmt = $this->pdo->prepare("SELECT COUNT(*) as total FROM borrowings WHERE status = 'pending'");
        $stmt->execute();
        $stats['pending_requests'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        
        // Overdue books
        $stmt = $this->pdo->prepare("SELECT COUNT(*) as total FROM borrowings WHERE status = 'approved' AND due_date < ?");
        $stmt->execute([date('Y-m-d')]);
        $stats['overdue_books'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        
        // User-specific stats
        if ($_SESSION['user_role'] === 'student') {
            $stmt = $this->pdo->prepare("SELECT COUNT(*) as total FROM borrowings WHERE student_id = ? AND status = 'approved'");
            $stmt->execute([$_SESSION['user_id']]);
            $stats['my_borrowed_books'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
            
            $stmt = $this->pdo->prepare("SELECT COUNT(*) as total FROM download_history WHERE student_id = ?");
            $stmt->execute([$_SESSION['user_id']]);
            $stats['my_downloads'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        }
    ...
    public function getRecentBooks($limit = 5) {
        $stmt = $this->pdo->prepare("SELECT * FROM books ORDER BY created_at DESC LIMIT ?");
        $stmt->execute([$limit]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    
    public function getPopularBooks($limit = 5) {
        $sql = "SELECT b.*, COUNT(dh.id) as download_count
                FROM books b
                LEFT JOIN download_history dh ON b.id = dh.book_id
                GROUP BY b.id
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'get_recent_books':
            $books = $dashboard->getRecentBooks();
            jsonResponse(true, '', $books);
            break;
            
        case 'get_popular_books':
            $books = $dashboard->getPopularBooks();
            jsonResponse(true, '', $books);
            break;
