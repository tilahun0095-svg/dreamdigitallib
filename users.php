...
class UserManager {
    ...
    public function getAllUsers($filters = []) {
        ...
        $sql = "SELECT id, stud_id, full_name, age, sex, department, email, role, created_at 
    ...
    public function addUser($data) {
        ...
        if ($data['role'] === 'student') {
            ...
            $studentFields = ['age', 'sex', 'department'];
            foreach ($studentFields as $field) {
                if (empty($data[$field])) {
                    jsonResponse(false, "Field $field is required for students");
                }
            }
        }
        ...
        // Insert user
        $stmt = $this->pdo->prepare("INSERT INTO users (stud_id, full_name, age, sex, department, email, password, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        
        if ($stmt->execute([
            $stud_id, $data['full_name'], $data['age'] ?? null, $data['sex'] ?? null,
            $data['department'] ?? null, $data['email'], $hashedPassword, $data['role']
        ])) {
    ...
    public function updateUser($id, $data) {
        ...
        $fields = ['full_name', 'email', 'age', 'sex', 'department', 'role'];
    ...
    public function resetPassword($id) {
        ...
        if ($stmt->execute([$hashedPassword, $id])) {
            jsonResponse(true, 'Password reset successfully. New password: ' . $newPassword);
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'update_user':
            ...
            $userManager->updateUser($_POST['id'], $_POST);
            break;
        ...
        case 'reset_password':
            ...
            $userManager->resetPassword($_POST['id']);
            break;
